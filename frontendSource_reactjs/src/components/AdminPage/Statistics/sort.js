export const calculateDay = (minusDay) => {
    ...
    if (day[0] === '0' && parseInt(day[1]) > minusDay) {// 08 bolgan holat
        ...
        if (parseInt(month[0]) === 0) {
            ...
            if (parseInt(month[1]) === 3 && parseInt(year) % 4 === 0) {
                forCompare = [year, `${parseInt(month) - 1}`, 29 + parseInt(day[1]) - minusDay];
            } else if (parseInt(month[0]) === 3) {
                forCompare = [year, `${parseInt(month) - 1}`, 28 + parseInt(day[1]) - minusDay];
            } else if (parseInt(month[1]) === 10 || parseInt(month[1]) === 12) {
                forCompare = [year, `${parseInt(month) - 1}`, 30 + parseInt(day[1]) - minusDay];
            } else if (parseInt(month[1]) === 11) {
                forCompare = [year, `${parseInt(month) - 1}`, 31 + parseInt(day[1]) - minusDay];
            }
        }
    } else {
        //demak sana 15 korinishida
        forCompare = [year, month, parseInt(day) - minusDay];
    }

    forCompare[2] = forCompare[2] > 9 ? forCompare[2] : `0${forCompare[2]}`;
    // forCompare[1] = parseInt(forCompare[1]) > 9 ? forCompare[1] : `0${forCompare[1]}`;
